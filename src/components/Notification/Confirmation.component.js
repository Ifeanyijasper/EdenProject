    ...
    return (
        <>
            <div onClick={() => setConfirm(false)} className={`fixed z-50 w-full h-full bg-white bg-opacity-10 fixed z-20 backdrop-filter backdrop-blur-sm transition-all duration-500 ease-in-out ${confirm ? 'inset-0' : '-top-full left-0'}`}>
                ...
                >
                    <h2>{data?.msg} <br /> {data?.data?.fullname ? data?.data?.fullname : data?.data?.username}{data?.data?.name} {data?.data?.client && `Purchase by ${data?.data?.client} served by ${data?.data?.worker}. Total: ${Thousand(data?.data?.total)} XAF`}</h2>
