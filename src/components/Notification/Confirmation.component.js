    ...
    return (
        <>
            <div onClick={() => setConfirm(false)} className={`fixed z-50 w-full h-full bg-white bg-opacity-10 fixed z-20 backdrop-filter backdrop-blur-sm transition-all duration-500 ease-in-out ${confirm ? 'inset-0' : '-top-full left-0'}`}>
                <div
                    onClick={(e) => stop(e)}
                    className={`absolute capitalize shadow-xl p-4 w-72 text-wrap text-center font-semibold rounded-md bg-green-700 bg-opacity-90 left-1/2 top-1/2 transform -translate-y-1/2 -translate-x-1/2 text-white z-40 text-sm transition-all duration-500 ease-in-out`}
                >
                    <h2>{data?.msg} {data?.data?.fullname ? data?.data?.fullname : data?.data?.username}{data?.data?.name} {data?.data?.client && `Purchase by ${data?.data?.client} served by ${data?.data?.worker}`}</h2>
